...
def openml_scores(
    *, 
    ...
    print(f"RUNNING MORTALITY FOR seed={seed}, imputer={imputer_name}")

    # Read in mimic mortality data
    train_data = pd.read_csv("../mimic_data/mortality_tab_train.csv")
    X_train = train_data[[c for c in train_data.columns if c != "target"]].copy(deep=False)
    X_train = X_train[[c for c in X_train.columns if "full" in c and "mean" in c]]
    y_train = train_data["target"].copy(deep=False)

    test_data = pd.read_csv("../mimic_data/mortality_tab_test.csv")
    ...
    # Gets tasks and models for binary classification tasks
    ...
    for model, model_params in zip(models, model_params_sets):
        ...
        smim_score = metric(y_test, model_.predict_proba(X_test_input)[:, 1], **metric_kwargs)
        results.append([seed, alpha, imputer_name, model_name, "SMIM", smim_score, impute_time, smim_time])

    print(f"FINISHING MORTALITY FOR seed={seed}, imputer={imputer_name}")
    return results
...
df.to_csv(f"/data/jmv249/Informative_Missingness/mimic_outputs/mortality.csv", index=False)