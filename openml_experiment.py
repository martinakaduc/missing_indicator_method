...
def openml_scores(*, imputer_name, dataset_name, seed=10, alpha=0.05, inf_prob=0.5):
    ...
    if imputer_name in ["lrgc", "gc"] and dataset_name in ["dilbert", "arcene"]:
        results = []
        for model, model_params in zip(models, model_params_sets):
            model_name = get_model_name(model)
            results.append(
    ...
    X_train_imputed, impute_time = time_fit_transform(
        imputer, X_train_masked, y_train, time_limit=60 * 60 * 3
    )
    ...
    for model, model_params in zip(models, model_params_sets):
        ...
        oracle_mim = Oracle_MIM(indicators_to_keep=np.flatnonzero(power))
