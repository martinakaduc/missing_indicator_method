...
def process_partition(
    ...
    patients = list(
        filter(str.isdigit, os.listdir(os.path.join(args.root_path, partition)))
    )

    for patient in tqdm(
        patients, desc="Iterating over patients in {}".format(partition)
    ):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as ts_file:
                ...
                cur_los_masks = map(
                    int,
                    (sample_times > shortest_length) & (sample_times > event_times[0]),
                )
                ...
                diagnoses_df = pd.read_csv(
                    os.path.join(patient_folder, "diagnoses.csv"),
                    dtype={"ICD9_CODE": str},
                )
                ...
                cur_phenotype_labels = [
                    x
                    for (i, x) in enumerate(cur_phenotype_labels)
                ...
                if pd.isnull(deathtime):
                    ...
                    lived_time = (
                        datetime.strptime(deathtime, "%Y-%m-%d %H:%M:%S")
                        - datetime.strptime(intime, "%Y-%m-%d %H:%M:%S")
                    ).total_seconds() / 3600.0

                sample_times = np.arange(0.0, min(los, lived_time) + eps, sample_rate)
                sample_times = np.array([int(x + eps) for x in sample_times])
                cur_decomp_masks = map(
                    int,
                    (sample_times > shortest_length) & (sample_times > event_times[0]),
                )
                cur_decomp_labels = [
                    (mortality & int(lived_time - t < future_time_interval))
    ...
    with open(os.path.join(output_dir, "listfile.csv"), "w") as listfile:
        header = ",".join(
            [
                "filename",
                "length of stay",
                "in-hospital mortality task (pos;mask;label)",
                "length of stay task (masks;labels)",
                "phenotyping task (labels)",
        ...
        for index in range(len(file_names)):
            ...
            ihm_task = "{:d};{:d};{:d}".format(
                ihm_positions[index], ihm_masks[index], ihm_labels[index]
            )
            ...
            listfile.write(
                ",".join([file_name, los, ihm_task, los_task, pheno_task, decomp_task])
                + "\n"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create data for multitask prediction."
    )
    parser.add_argument(
        "root_path",
        type=str,
    ...
    process_partition(
        args, definitions, code_to_group, id_to_group, group_to_id, "test"
    )
    process_partition(
        args, definitions, code_to_group, id_to_group, group_to_id, "train"
    )
