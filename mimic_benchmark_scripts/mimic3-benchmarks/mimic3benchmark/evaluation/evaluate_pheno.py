...
def main():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), "../../data/phenotyping/test/listfile.csv"
        ),
    )
    ...
    pred_df = pd.read_csv(
        args.prediction, index_col=False, dtype={"period_length": np.float32}
    )
    test_df = pd.read_csv(
        args.test_listfile, index_col=False, dtype={"period_length": np.float32}
    )
    ...
    for i in range(1, n_tasks + 1):
        ...
        results[m] = dict()
        results[m]["value"] = print_metrics_binary(
            data[:, 25 + i - 1], data[:, i - 1], verbose=0
        )["auroc"]
        results[m]["runs"] = []
    ...
    for iteration in range(args.n_iters):
        ...
        for i in range(1, n_tasks + 1):
            m = "ROC AUC of task {}".format(i)
            cur_auc = print_metrics_binary(
                cur_data[:, 25 + i - 1], cur_data[:, i - 1], verbose=0
            )["auroc"]
    ...
    print(
        "Saving the results (including task specific metrics) in {} ...".format(
            args.save_file
        )
    )
