...
def main():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), "../../data/length-of-stay/test/listfile.csv"
        ),
    )
    ...
    pred_df = pd.read_csv(
        args.prediction,
        index_col=False,
        dtype={"period_length": np.float32, "y_true": np.float32},
    )
    test_df = pd.read_csv(
        args.test_listfile,
        index_col=False,
    ...
    metrics = [("Kappa", "kappa"), ("MAD", "mad"), ("MSE", "mse"), ("MAPE", "mape")]
