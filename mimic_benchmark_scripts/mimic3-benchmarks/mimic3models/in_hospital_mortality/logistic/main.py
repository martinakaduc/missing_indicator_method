...
def main():
    ...
    (train_X, train_y, train_names) = read_and_extract_features(
        train_reader, args.period, args.features
    )
    (val_X, val_y, val_names) = read_and_extract_features(
        val_reader, args.period, args.features
    )
    (test_X, test_y, test_names) = read_and_extract_features(
    ...
    imputer = Imputer(
        missing_values=np.nan, strategy="mean", axis=0, verbose=0, copy=True
    )
    imputer.fit(train_X)
    ...
    with open(
        os.path.join(result_dir, "train_{}.json".format(file_name)), "w"
    ) as res_file:
    ...
    with open(
        os.path.join(result_dir, "val_{}.json".format(file_name)), "w"
    ) as res_file:
    ...
    with open(
        os.path.join(result_dir, "test_{}.json".format(file_name)), "w"
    ) as res_file:
