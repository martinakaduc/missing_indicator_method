...
def main():
    ...
    parser.add_argument(
        "dataset_dir", type=str, help="Path to the directory which contains the dataset"
    )
    args, _ = parser.parse_known_args()

    val_patients = set()
    with open(
        os.path.join(os.path.dirname(__file__), "resources/valset.csv"), "r"
    ) as valset_file:
    ...
    with open(
        os.path.join(args.dataset_dir, "train_listfile.csv"), "w"
    ) as train_listfile:
