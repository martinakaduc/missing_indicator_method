...
suffix = "{}.bs{}{}{}.ts{}".format(
    "" if not args.deep_supervision else ".dsup",
    args.batch_size,
...
model.compile(optimizer=optimizer_config, loss="binary_crossentropy")
model.summary()
...
if args.mode == "train":
    ...
    path = os.path.join(
        args.output_dir,
        "keras_states/" + model.final_name + ".chunk{epoch}.test{val_loss}.state",
    )
    ...
    if args.deep_supervision:
        ...
        for i in range(test_data_gen.steps):
            ...
            for m, t, p, name in zip(
                x[1].flatten(), y.flatten(), pred.flatten(), cur_names.flatten()
            ):
                if np.equal(m, 1):
                    labels.append(t)
        ...
        test_data_gen = utils.BatchGen(
            ...
            shuffle=False,
            return_names=True,
        )  # put steps = None for a full test
    ...
    path = (
        os.path.join(
            args.output_dir, "test_predictions", os.path.basename(args.load_state)
        )
        + ".csv"
    )
