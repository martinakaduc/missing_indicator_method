...
parser.add_argument(
    "--data",
    type=str,
    help="Path to the data of multitasking",
...
if normalizer_state is None:
    normalizer_state = "mult_ts{}.input_str:{}.start_time:zero.normalizer".format(
...
optimizer_config = {
    ...
    "config": {"lr": args.lr, "beta_1": args.beta_1},
}
...
model.compile(optimizer=optimizer_config, loss=loss_dict, loss_weights=loss_weights)
model.summary()
...
if args.mode == "train":
    ...
    metrics_callback = keras_utils.MultitaskMetrics(
    ...
    csv_logger = CSVLogger(
        os.path.join(keras_logs, model.final_name + ".csv"), append=True, separator=";"
    )
    ...
    for i in range(test_data_gen.steps):
        ...
        # ihm
        for m, t, p, name in zip(
            ihm_M.flatten(), ihm_t.flatten(), ihm_p.flatten(), names
        ):
            if np.equal(m, 1):
                ihm_y_true.append(t)
                ihm_pred.append(p)
        ...
        for x in ret["decomp_ts"]:
            decomp_ts += x
        for name, m, t, p in zip(
            names_extended.flatten(),
            decomp_M.flatten(),
            decomp_t.flatten(),
            decomp_p.flatten(),
        ):
    ...
    # ihm
    ihm_path = os.path.join(
        os.path.join(
            args.output_dir, "test_predictions/ihm", os.path.basename(args.load_state)
        )
        + ".csv"
    )
    ...
    decomp_path = os.path.join(
        os.path.join(
            args.output_dir,
            "test_predictions/decomp",
            os.path.basename(args.load_state),
        )
        + ".csv"
    )
    decomp_utils.save_results(
        decomp_names, decomp_ts, decomp_pred, decomp_y_true, decomp_path
    )

    # los
    los_path = os.path.join(
        os.path.join(
            args.output_dir, "test_predictions/los", os.path.basename(args.load_state)
        )
        + ".csv"
    )
    ...
    pheno_path = os.path.join(
        os.path.join(
            args.output_dir, "test_predictions/pheno", os.path.basename(args.load_state)
        )
        + ".csv"
    )
    pheno_utils.save_results(
        pheno_names, pheno_ts, pheno_pred, pheno_y_true, pheno_path
    )
