...
def openml_scores(
    *,
    ...
    print(f"RUNNING PHENOTYPING FOR seed={seed}, imputer={imputer_name}")

    # Read in mimic phenotyping data
    X_train = pd.read_csv("../mimic_data/phenotyping_tab_train.csv")
    X_train = X_train[[c for c in X_train.columns if "full" in c and "mean" in c]]
    y_train = pd.read_csv("../mimic_data/phenotyping_train_labels.csv")

    X_test = pd.read_csv("../mimic_data/phenotyping_tab_test.csv")
    X_test = X_test[[c for c in X_test.columns if "full" in c and "mean" in c]]
    y_test = pd.read_csv("../mimic_data/phenotyping_test_labels.csv")
    ...
    for model, model_params in zip(models, model_params_sets):
        ...
        # train_mask_feats = smim.fit_transform(X_train.to_numpy(), y_train.to_numpy())
        # test_mask_feats = smim.transform(X_test.to_numpy())
