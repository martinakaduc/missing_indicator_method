...
from models.imputation_models import GCImputer, LRGCImputer, MIM, Oracle_MIM
...
def gen_results(n, p, imputer_name, n_blocks=10, inf_prob=0.5, seed=10, alpha=0.05):
    ...
    (X_train, X_test, latent_X_train, latent_X_test, y_train, y_test) = (
    ...
    if imputer_name == "mean":
        ...
        imputer = make_pipeline(
            StandardScaler(),
            LRGCImputer(
                random_state=seed, rank=10, min_ord_ratio=np.inf, max_iter=50, n_jobs=1
            ),
        )
    ...
    # Evaluate each model for no MIM, MIM, and dynamic MIM
    results = []
    for model, model_params in zip(models, model_params_sets):
        model_name = get_model_name(model)
        # print(model_name)
        ...
        train_mask_feats = dynamic_mim.fit_transform(
            X_train_masked, y_train, X_imputed=X_train_imputed
        )
        test_mask_feats = dynamic_mim.transform(X_test_masked)
...
if __name__ == "__main__":
    ...
    def test():
        results = Parallel(n_jobs=args.n_jobs, backend="multiprocessing")(
            ...
            for seed, inf_prob in itertools.product(args.seeds, args.inf_probs)
        )
        return results
